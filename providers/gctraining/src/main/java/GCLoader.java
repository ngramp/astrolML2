...
public class GCLoader extends NativeImageLoader implements Serializable{
    ...
    final static String BASE_DIR = FilenameUtils.concat(System.getProperty("user.dir"), "src/main/resources/");
    private final static String LOCAL_TRAIN_DIR = "train";
    private final static String LOCAL_VAL_DIR = "test";
    final static String CLS_TRAIN_ID_TO_LABELS = "cls-loc-labels.txt";
    final static String CLS_VAL_ID_TO_LABELS = "cls-loc-val-map.txt";
    private String urlTrainFile = "image_train_urls.txt";
    private String urlValFile = "image_test_urls.txt";
    private String labelFilePath;

    private List<String> labels = new ArrayList<>();
    private Map<String,String> labelIdMap = new LinkedHashMap<>();

    private File fullTrainDir = new File(BASE_DIR, LOCAL_TRAIN_DIR);
    private File fullTestDir = new File(BASE_DIR, LOCAL_VAL_DIR);
    private File sampleURLTrainList = new File(BASE_DIR, urlTrainFile);
    private File sampleURLTestList = new File(BASE_DIR, urlValFile);
    ...
    private int maxExamples2Label;
    private PathLabelGenerator labelGenerator;
    private double splitTrainTest;
    private Random rng;

    private DataModeEnum dataModeEnum; // CLS_Train, CLS_VAL, CLS_TEST, DET_TRAIN, DET_VAL, DET_TEST
    private final static String REGEX_PATTERN = Pattern.quote("_");
    private final static PathLabelGenerator LABEL_PATTERN = new PatternPathLabelGenerator(REGEX_PATTERN);
    private RecordReader recordReader;

    public GCLoader(int batchSize, int numExamples, int numLabels, int maxExamples2Label, @Null PathLabelGenerator labelGenerator, DataModeEnum dataModeEnum, @Null double splitTrainTest, @Null Random rng, @Null File localDir){
...
//            downloadAndUntar(generateMaps(fileName, url.toString()), fullDir);
    ...
    public RecordReader getRecordReader(int[]imgDim, ImageTransform imageTransform, int normalizeValue) {
        load();
        recordReader = new GCRecordReader(imgDim[0], imgDim[1], imgDim[2], labelGenerator, imageTransform, normalizeValue, dataModeEnum);
